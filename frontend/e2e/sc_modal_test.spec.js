...
async function createSCViaUI(page) {
    ...
    // Item — silver cert has Gross Wt + Test Wt (no rate)
    await modal.locator('input[placeholder="e.g. RING, NECK"]').fill('CHAIN');
    const numInputs = modal.locator('input[type="number"]');
    await numInputs.nth(0).fill('15.000');  // gross weight
    await numInputs.nth(1).fill('0.300');   // test weight
...
test.describe('Silver Certificate — modal flow', () => {

    test('1. Create SC via New Silver Cert modal', async ({ page }) => {
        await login(page);
        await goToSCTab(page);

        const countBefore = await page.locator('.kanban-column').nth(0).locator('.kanban-card').count();
        await createSCViaUI(page);
    ...
    test('2. Phase2Modal opens with "Add Certificate Results" title for TODO silver cert', async ({ page }) => {
        await login(page);
        await goToSCTab(page);
        await createSCViaUI(page);
        await page.waitForTimeout(600);
    ...
    test('4. Finalize (IN_PROGRESS → DONE) persists mode_of_payment', async ({ page }) => {
        ...
        console.log('✅  Amount=400, Mode=UPI entered');
        ...
        // Verify via API
        const listRes = await page.request.get(`${API}/certificates?type=silver&status=DONE&limit=1`);
        const listBody = await listRes.json();
        const latestDone = (listBody.certificates || [])[0];
        if (latestDone) {
            const certRes = await page.request.get(`${API}/certificates/${latestDone.id}?type=silver`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const cert = await certRes.json();
            console.log('Latest DONE silver cert — status:', cert.status, '| mode:', cert.mode_of_payment);
            expect(cert.status).toBe('DONE');
            expect(cert.mode_of_payment).toBe('UPI');
            console.log('✅  mode_of_payment=UPI persisted in DB');
        } else {
            console.log('ℹ️  No DONE silver cert found via API — skipping DB check');
        }
    });

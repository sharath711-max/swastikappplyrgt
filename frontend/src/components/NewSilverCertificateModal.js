...
const MAX_ITEMS = 20;
const WORKFLOW_TYPE = 'SC';

// Sample Returned defaults to CHECKED (Python parity)
const emptyRow = () => ({
...
const NewSilverCertificateModal = ({ show, onHide, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await runModalSubmit({
                action: async () => {
                    if (!preventDuplicateCreate('SC', selectedCustomer.id))
                        throw new Error('Duplicate silver certificate submission blocked');
                    ...
                    addToast(`Silver Certificate created — ${sampleRows.length} item(s)`, 'success');
                },
                reload: onSuccess,
            ...
            if (err.message === 'Duplicate silver certificate submission blocked') {
                addToast('Silver Certificate creation is already in progress', 'warning');
                return;
            }
            addToast(err?.response?.data?.error || 'Failed to create certificate', 'error');
        } finally {
    ...
    return (
        ...
        <Modal show={show} onHide={closeSafely} backdrop="static" centered dialogClassName="modal-xxl">
            <Modal.Header closeButton>
                <Modal.Title as="h3" id="newCertModalTitle">New Silver Certificate</Modal.Title>
            </Modal.Header>

            <Modal.Body className="m-3 mb-0" onKeyDown={onEnterAdvance}>

                <PrerequisiteBanner>
                    Silver Certificate is typically generated when a Silver Test is finalized.
                    Direct entry is supported for legacy or off-system items but skips test-result reuse.
                </PrerequisiteBanner>
                        ...
                        <Form id="addCustomerForm" onSubmit={saveNewCustomer} autoComplete="off" noValidate>
                            ...
                            <div className="d-flex justify-content-end">
                                <Button id="addCustomerSubmitBtn" type="submit" variant="primary" className="m-1" disabled={savingCust}>
                                    {savingCust && (<span className="spinner-border spinner-border-sm me-2" role="status" />)}
                                    Add
                                </Button>
                            </div>
                        </Form>
...
export default NewSilverCertificateModal;

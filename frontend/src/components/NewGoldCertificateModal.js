...
// Sample Returned defaults to CHECKED for GC (matches Python: `<input ... name="returned" checked>`)
const emptyRow = () => ({
...
const NewGoldCertificateModal = ({ show, onHide, onSuccess }) => {
    ...
    // Sample rows + GST flag — multi-row inline editing
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await runModalSubmit({
                action: async () => {
                    if (!preventDuplicateCreate('GC', selectedCustomer.id))
                        throw new Error('Duplicate gold certificate submission blocked');
                    ...
                    // Backend `/certificates/with-photo` accepts JSON payload via
                    // multipart `data` field. No photo for GC — only PC sends one.
                    ...
                    addToast(`Gold Certificate created — ${sampleRows.length} item(s)`, 'success');
                },
                reload: onSuccess,
                close: () => {
                    submitReqRef.current = null;
            ...
            if (err.message === 'Duplicate gold certificate submission blocked') {
                addToast('Gold Certificate creation is already in progress', 'warning');
                return;
            }
...
export default NewGoldCertificateModal;

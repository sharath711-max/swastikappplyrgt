...
import NewPhotoCertificateModal from '../../../components/NewPhotoCertificateModal';
import { ModalProvider } from '../../../contexts/ModalContext';

const mockCustomers = [
    { id: 'CUS-001', name: 'Acme Jewellers', phone: '9876543210', balance: 3200 },
    { id: 'CUS-002', name: 'Beta Photo',     phone: '9123456780', balance: 0 },
];
...
const renderModal = (props = {}) => {
    ...
    const result = render(
        <ModalProvider>
            <NewPhotoCertificateModal show={true} onHide={onHide} onSuccess={onSuccess} {...props} />
        </ModalProvider>
    );
...
describe('PC Modal — Group A: Row entry', () => {
    test('A1: Title shows "New Photo Certificate"', () => {
        renderModal();
        expect(screen.getByText(/new photo certificate/i)).toBeInTheDocument();
    });
    ...
    test('A4: Sample Returned defaults to CHECKED (Python PC parity)', async () => {
        const user = userEvent.setup();
        renderModal();
        await selectFirstCustomer(user);
    ...
    test('A5: Each row has a photo file input (Python PC parity)', async () => {
        const user = userEvent.setup();
        renderModal();
        await selectFirstCustomer(user);
        await clickAddSampleBtn(user);

        const fileInputs = document.querySelectorAll('#sampleDetailsContainer input.photo[type="file"]');
        expect(fileInputs.length).toBe(2);
        fileInputs.forEach(fi => expect(fi).toHaveAttribute('accept', 'image/*'));
    });
});

describe('PC Modal — Group B: GST checkbox', () => {
    ...
    test('B2: GST flag flows to payload as photo type', async () => {
        const user = userEvent.setup();
        renderModal();
        await selectFirstCustomer(user);
        ...
        expect(payload.type).toBe('photo');
    });
});

describe('PC Modal — Group C: Submit flow', () => {
    test('C1: Submit without customer → no button', () => {
        renderModal();
        expect(document.querySelector('#sampleDetailsSubmitBtn')).toBeNull();
    });

    test('C2: Submit single row → POST with FormData and X-Request-Id', async () => {
        const user = userEvent.setup();
        renderModal();
        await selectFirstCustomer(user);
        await fillRow(user, 0, { name: 'Idol', item: 'Statue', total: '100', test: '5.0' });
        await clickSubmit(user);
        await waitFor(() => expect(mockApiPost).toHaveBeenCalledTimes(1));
        ...
        expect(payload.items[0]).toMatchObject({
            item_type: 'Statue', gross_weight: 100, test_weight: 5.0, returned: true,
        });
    });

    test('C3: First row photo attaches to FormData (single-photo backend constraint)', async () => {
        const user = userEvent.setup();
        renderModal();
        await selectFirstCustomer(user);
        await fillRow(user, 0, { item: 'Statue', total: '100', test: '5.0' });

        // Attach a photo file to row 0
        const fileInput = document.querySelector('#sampleDetailsContainer input.photo[type="file"]');
        const file = new File(['test-image-bytes'], 'idol.jpg', { type: 'image/jpeg' });
        await user.upload(fileInput, file);

        await clickSubmit(user);
        await waitFor(() => expect(mockApiPost).toHaveBeenCalledTimes(1));
        const [, formData] = mockApiPost.mock.calls[0];
        const attachedPhoto = formData.get('photo');
        expect(attachedPhoto).toBeInstanceOf(File);
        expect(attachedPhoto.name).toBe('idol.jpg');
    });
});

describe('PC Modal — Group D: Validation', () => {
...
describe('PC Modal — Group E: Customer flow', () => {
    test('E1: previousCertificate placeholder exists', () => {
        renderModal();
        expect(document.querySelector('#previousCertificate')).not.toBeNull();
    });

    test('E2: AddCustomer has Notes textarea', () => {
        renderModal();

...
export default function ItemsPage() {
  ...
  // One request ID per pending save batch — reset on successful save
  ...
  // Reload list when metal changes
  useEffect(() => {
  ...
  // Call backend calculation on blur — preserves "no business logic in UI" rule
  const handleCalcBlur = async (itemId) => {
    ...
    try {
      ...
      // Preview failure is non-blocking — server values still show
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      saveReqIdRef.current = createRequestId();
      setDirty(new Set());
      // Refresh to get server-canonical computed values
                ...
                <Table responsive hover size="sm" className="mb-0 align-middle small">
                  ...
                  <tbody>
                    {rows.map((row, idx) => {
                      ...
                      const netDisplay = preview.net_weight != null
                        ? fmtW(preview.net_weight)
                        : fmtW(row.net_weight);
                      const fineDisplay = preview.fine_weight != null
                        ? fmtW(preview.fine_weight)
                        : fmtW(row.fine_weight);
                      ...
                      return (
                        <tr key={row.id} className={isDirty ? 'table-warning' : ''}>
                          ...
                          <td className={hasPreview ? 'text-primary fw-semibold' : ''}>
                            {isCalc
                              ? <Spinner style={{ width: 12, height: 12 }} animation="border" />
                              : netDisplay}
                          </td>
